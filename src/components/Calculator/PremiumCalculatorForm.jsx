...
function PremiumCalculatorForm({ onSubmit, defaultPlatform = '', disabled = false }) {
  ...
  const [formData, setFormData] = useState({
    ...
    followers: '',
    // New detailed sponsor fields (OPTIONAL in PRO)
    campaignType: '',
    partnershipDuration: '',
    exclusivity: '',
    usageRights: '',
    // Partnership end date (optional reminder)
    partnership_end_date: '',
    // Brand offer (optional in PRO)
    offeredPrice: '',
  })
  ...
  const handleChange = (e) => {
    ...
    if (['subscribers', 'averageViews', 'averageConcurrentViewers', 'followers', 'offeredPrice'].includes(name)) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white shadow-sm rounded-xl p-6 border border-amber-200 relative">
      {/* PRO Badge */}
      <div className="absolute top-4 right-4">
        <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-400 to-orange-500 text-white rounded-full text-xs font-bold shadow-md">
          <Crown size={14} />
          <span>{t('calculator.pro_badge')}</span>
        </div>
      </div>

      <div className="space-y-4 mt-8">
        {/* Brand Offer Price (optional in PRO) */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t('calculator_form.labels.brand_offer_price')} ({currencySymbol})
          </label>
          <input
            type="number"
            name="offeredPrice"
            ...
            disabled={disabled}
            placeholder={t('calculator_form.placeholders.brand_offer_placeholder')}
          ...
          <p className="text-xs text-gray-500 mt-1">
            {t('calculator_form.hints.brand_offer_leave_empty')}
          </p>
        </div>
        {/* Platform */}
        <div>
...
<label htmlFor="engagementRate" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <p className="mt-1 text-xs text-gray-500">{t('calculator_form.hints.engagement_formula')}</p>
              </div>
        ...
        <div>
          <label htmlFor="companySize" className="block text-sm font-medium text-gray-700 mb-1">
            {t('calculator_form.labels.company_size_target')}
          </label>
          <select
            id="companySize"
            name="companySize"
        ...
        {/* Section: About the Sponsor (OPTIONAL in PRO) */}
        <div className="border-t border-gray-200 pt-6 mt-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg font-semibold text-gray-900">
              {t('calculator_form.labels.about_sponsor')}
            </h3>
            <span className="text-sm text-premium-600 font-medium">({t('calculator_form.labels.recommended')})</span>
          </div>
          <p className="text-sm text-gray-600 mb-4">
            {t('calculator_form.hints.about_sponsor_fill')}
          </p>
          ...
          <div className="mb-4">
            ...
            >
              <option value="">{t('calculator_form.placeholders.select_campaign_type_optional')}</option>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="">{t('calculator_form.placeholders.select_partnership_duration_optional')}</option>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="">{t('calculator_form.placeholders.select_exclusivity_optional')}</option>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="">{t('calculator_form.placeholders.select_usage_rights_optional')}</option>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t('calculator_form.labels.partnership_end_date')} <span className="text-gray-400 font-normal">({t('calculator_form.labels.optional')})</span>
          </label>
          <input
            type="date"
            name="partnership_end_date" // Note: snake_case pour matcher directement la DB
            value={formData.partnership_end_date || ''}
            onChange={handleChange}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all"
          />
          <p className="text-xs text-gray-500 mt-1">
            {t('calculator_form.hints.partnership_end_reminder')}
          </p>
        </div>
        ...
        <button
          type="submit"
          disabled={disabled}
          className="w-full bg-gradient-to-r from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 text-white font-semibold py-3 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg flex items-center justify-center gap-2"
        >
          <Sparkles size={20} />
          {disabled ? t('calculator_form.buttons.calculating') : t('calculator_form.buttons.calculate')}
        </button>
      </div>
    </form>
  )
